...
public class CloudBalancingIncrementalScoreCalculator extends AbstractIncrementalScoreCalculator<CloudBalance> {
    ...
    private Map<CloudComputer, Integer> Flexible_timeUsageMap;
    ...
    public void resetWorkingSolution(CloudBalance cloudBalance) {
        ...
        Flexible_timeUsageMap = new HashMap<>(computerListSize);
    ...
    private void insert(CloudProcess process) {
        ...
        if (computer != null) {
            ...
            int Flexible_time = computer.getFlexible_time();
            int oldFlexible_timeUsage =Flexible_timeUsageMap.get(computer);
            int oldFlexible_timeAvailable = Flexible_time - oldFlexible_timeUsage;
            int newFlexible_timeUsage = oldFlexible_timeUsage + process.getProcessing_time();
            int newFlexible_timeAvailable = Flexible_time - newFlexible_timeUsage;
            hardScore += Math.min(newFlexible_timeAvailable, 0) - Math.min(oldFlexible_timeAvailable, 0);
            Flexible_timeUsageMap.put(computer, newFlexible_timeUsage);
