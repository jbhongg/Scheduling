...
public class CloudBalancingGenerator extends LoggingMain {
    ...
    private static final Price[] FLEXIBLE_TIME_PRICES = { // in gigabyte per hour
            new Price(2, "2 hour", 120),
            new Price(4, "4 hour", 180),
            new Price(6, "6 hour", 240),
            new Price(8, "8 hour", 360),
            new Price(12, "12 hour", 500),
            new Price(16, "16 hour", 800),
            new Price(20, "20 hour", 1000),
    };
    ...
    private static final int MAXIMUM_PROCESSING_TIME = 12;
...
//        generator.writeCloudBalance(20, 60);
    ...
    private void checkConfiguration() {
        if (CPU_POWER_PRICES.length != MEMORY_PRICES.length || CPU_POWER_PRICES.length != NETWORK_BANDWIDTH_PRICES.length || CPU_POWER_PRICES.length != FLEXIBLE_TIME_PRICES.length) {
    ...
    public CloudComputer generateComputerWithoutId() {
        ...
        int Flexible_timePricesIndex = distortIndex(cpuPowerPricesIndex, FLEXIBLE_TIME_PRICES.length);
        computer.setFlexible_time(FLEXIBLE_TIME_PRICES[Flexible_timePricesIndex].getHardwareValue());
        int cost = CPU_POWER_PRICES[cpuPowerPricesIndex].getCost()
                ...
                + FLEXIBLE_TIME_PRICES[Flexible_timePricesIndex].getCost();
        computer.setCost(cost);
        logger.trace("Created computer with cpuPowerPricesIndex ({}), memoryPricesIndex ({}),"
                + " networkBandwidthPricesIndex ({}), + Flexible_timePricesIndex ({}).",
                cpuPowerPricesIndex, memoryPricesIndex, networkBandwidthPricesIndex, Flexible_timePricesIndex);
        return computer;
    }
    ...
    public CloudProcess generateProcessWithoutId() {
        ...
        int Processing_time = generateRandom(MAXIMUM_PROCESSING_TIME);
        process.setProcessing_time(Processing_time);
        logger.trace("Created CloudProcess with requiredCpuPower ({}), requiredMemory ({}),"
                + " requiredNetworkBandwidth ({}), Processing_time ({}).",
                requiredCpuPower, requiredMemory, requiredNetworkBandwidth, Processing_time);
    ...
    private void assureComputerCapacityTotalAtLeastProcessRequiredTotal(CloudBalance cloudBalance) {
        ...
        for (CloudProcess process : cloudBalance.getProcessList()) {
            ...
            Processing_timeTotal += process.getProcessing_time();
        }
        ...
        int Flexible_timeLacking = Processing_timeTotal - Flexible_timeTotal;
        while (Flexible_timeLacking > 0) {
            CloudComputer computer = computerList.get(random.nextInt(computerList.size()));
            int upgrade = determineUpgrade(Flexible_timeLacking);
            computer.setFlexible_time(computer.getFlexible_time() + upgrade);
            Flexible_timeLacking -= upgrade;
        }
    }
