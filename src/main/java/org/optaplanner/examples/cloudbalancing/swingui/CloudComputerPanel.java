...
public class CloudComputerPanel extends JPanel {
    ...
    private JTextField Flexible_timeField;
    ...
    private CloudBar Processing_timeBar;
    ...
    private int getComputerFlexible_time() {
        return computer == null ? 0 : computer.getFlexible_time();
    }

    private void createTotalsUI() {
        JPanel labelAndDeletePanel = new JPanel(new BorderLayout(7, 0));
        if (computer != null) {
        ...
        Flexible_timeField = new JTextField("0 h / " + getComputerFlexible_time() + " h");
        Flexible_timeField.setEditable(false);
        Flexible_timeField.setEnabled(false);
        add(Flexible_timeField);
    ...
    private void createBarsUI() {
        ...
        Processing_timeBar = new CloudBar(getComputerFlexible_time(), cloudBalancingPanel.getMaximumComputerFlexible_time());
        Processing_timeBar.setEnabled(false);
        add(Processing_timeBar);
    ...
    public void update() {
        ...
        int usedProcessing_time = 0;
        Processing_timeBar.clearProcessValues();
        for (CloudProcess process : processList) {
            ...
            usedProcessing_time += process.getProcessing_time();
            Processing_timeBar.addProcessValue(process.getProcessing_time());
        }
        boolean used = processList.size() > 0;
        updateTotals(usedCpuPower, usedMemory, usedNetworkBandwidth, usedProcessing_time, used);
        updateBars(used);
    }

    private void updateTotals(int usedCpuPower, int usedMemory, int usedNetworkBandwidth, int usedProcessing_time, boolean used) {
        ...
        Flexible_timeField.setText(usedProcessing_time + " h / " + getComputerFlexible_time() + " h ");
        Flexible_timeField.setForeground(usedProcessing_time > getComputerFlexible_time() ? TangoColorFactory.SCARLET_3 : Color.BLACK);
        Flexible_timeField.setEnabled(used);
    ...
    private void updateBars(boolean used) {
        ...
        Processing_timeBar.setEnabled(used);
        Processing_timeBar.repaint();
    ...
    private class CloudProcessListDialog extends JDialog {
        ...
        private JPanel createHeaderPanel() {
            ...
            JLabel Flexible_timeLabel = new JLabel("Flexible time");
            headerPanel.add(Flexible_timeLabel);
            headerPanel.add(new JLabel(""));
            return headerPanel;
        }

        private JPanel createAssignmentsPanel() {
            JPanel assignmentsPanel = new JPanel(new GridLayout(0, 6));
            int colorIndex = 0;
            for (final CloudProcess process : processList) {
                JPanel labelAndDeletePanel = new JPanel(new BorderLayout(6, 0));
                ...
                JTextField Flexible_timeField = new JTextField(process.getProcessing_time() + " h");
                Flexible_timeField.setEditable(false);
                assignmentsPanel.add(Flexible_timeField);
