...
public class TaskAssigningPanel extends SolutionPanel<TaskAssigningSolution> {
    ...
    public void produceUpTo(final int producedTime) {
        ...
        final int baseDurationBudgetPerMachine = (producedTime - previousProducedTime);
        final int newTaskCount = getSolution().getMachineList().size() * baseDurationBudgetPerMachine / BASE_DURATION_AVERAGE;
        if (newTaskCount <= 0) {
        ...
        doProblemFactChange(scoreDirector -> {
            ...
            for (int i = 0; i < newTaskCount; i++) {
                ...
                task.setvolume(producingRandom.nextInt(4)*250+250);
                task.setColor(colors[producingRandom.nextInt(colors.length)]);
                task.setDuedate(Duedates[producingRandom.nextInt(Duedates.length)]);
